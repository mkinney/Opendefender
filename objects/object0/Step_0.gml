	...
	if (keyboard_check_pressed(global.button)) {
		...
		if (objectUI.smartbomb == 1) {
			if (global.smartbomb > 0) {
				...
				instance_create(x,y,object_flash);
				sound_stop(sound_baiterwarning);
...
if (dying == 1) {
	instance_destroy(object_enemybullet);
	speed = 0;
	with (shootables) {
		speed = 0
	}
...
// if we are using the jet, add the sound
if (jet == 1) {
	if (sound_isplaying(sound_thurst) == false) {
		sound_loop(sound_thurst);
	}
}
else {
