...
if ((instance_exists(object0)) and (object0.dying == 0)) {
	...
	if ((act == 0) and (speed == 0)) {
		motion_add(floor(random(360)), choose(1, 2, 3));
	}
	if (act == 1) {
		if (movdelay == 0) {
			movdelay = 1;
			...
			if (viewon == 1) {
				if (x <= target_x) {
					motion_add(0, movspeed+object0.hspeed);
				}
				if (x > target_x) {
					motion_add(180, movspeed+(object0.hspeed*-1));
				}
			}
			if (viewon == 0) {
				if x <= target_x {
					...
					if (object0.hspeed < 0) {
						motion_add(0, movspeed+(object0.hspeed * -1));
					}
				}
				if (x > target_x) {
					if (object0.hspeed >= 0) {
						motion_add(180, movspeed+object0.hspeed);
					}
					if (object0.hspeed < 0) {
						motion_add(180, movspeed+(object0.hspeed * -1));
					}
			...
			if (y <= target_y) {
				motion_add(270,choose(0.5, 1, 1.5));
			}
			if (y > target_y) {
				motion_add(90,choose(0.5, 1, 1.5));
			}
