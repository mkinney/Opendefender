if (viewon == 1) {
	...
	if (predictshoot == 0) {
		fr = point_distance(x, y, target_x, target_y) / (shootspeed - 1);
		if (object0.vspeed >= 0) {
		...
		with (dis) {
			motion_add(point_direction(x, y, other.id.zx, other.id.zy) + 7 - floor(random(14)), (other.id.shootspeed - 1));
			hspeed += object0.hspeed;
		}
	} else {
		var frr = instance_create(x, y, object_enemybullet);
		with (frr) {
			motion_add(other.id.a + 20 - floor(random(40)), choose(other.id.shootspeed - 1, other.id.shootspeed - 0.5, other.id.shootspeed, other.id.shootspeed + 0.5, other.id.shootspeed + 1));
		}
	}
	sound_stop(sound_enemyshoot);
	sound_play(sound_enemyshoot);
}
